...
const FavoriteProductComponent = (props: CustomProductFavoriteProps) => {
  ...
  return (
    <Card sx={{ w: 1, display: 'flex', justifyContent: 'flex-start' }}>
      <CardMedia sx={{ flexBasis: ' 5rem' }} image={product?.attributes?.mediaUrl} title={product?.attributes?.name} />
      <CardContent sx={{ w: 1 }}>
        <Typography gutterBottom variant='body2' component='div'>
          {product?.attributes?.name}
        </Typography>
        <Typography variant='body2' color='text.secondary'>
          {product?.attributes?.product_sub_categories?.data[0].attributes.name}
        </Typography>
      </CardContent>
    </Card>
  )
